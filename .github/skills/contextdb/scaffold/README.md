# contextdb — メタスキーマ駆動の仕様データ管理プロトタイプ
...
```
python contextdb/engine.py                 # 検証レポート + 統計（error で exit 1 → CI でブロック）
python contextdb/generate.py               # 検証 → documents/ の全文書を out/ に生成
python contextdb/generate.py requirement-spec  # 指定した文書定義だけ生成
python contextdb/diff.py <基準> [対象]      # リビジョン間の差分レポート（対象省略 = 作業ツリー）
python contextdb/diff.py --baselines       # ベースライン一覧
```

ベースライン = Git タグ。`git tag baseline/R1.0` で作成し、
`python contextdb/diff.py baseline/R1.0` で「ベースラインからの変更点一覧」が出る。
（この仕組みは contextdb/ が Git 管理されていることが前提。）
...
`--root` を省略した場合はカレントディレクトリの `.contextdb/`（`metamodel.yaml` を
持つもの）を自動的にデータルートにし、それも無ければツールと同じディレクトリ
（このサンプルデータ）にフォールバックする。プロジェクトのデータディレクトリを
`.contextdb` という名前で置けば `--root` の指定を省略できる。
...
（設計の全体像は `.contextdb/docs/standard-pack-design.md`。Phase 1 = テンプレート
...
- 解決順: `<データルート>/packs/<名前>/` → 環境変数 `CONTEXTDB_PACK_PATH` →
...
結果**になり、`contextdb engine` はそのマージ済みモデルでデータを検証する。
...
```bash
contextdb conform                # L1（メタモデル）+ L2（データ・文書）+ lock 照合
contextdb conform --for-baseline # ベースライン前提（status_rules）も検査
contextdb conform --frozen       # pack.lock 不一致を error 扱い（CI 用）
contextdb pack lock              # 解決結果から pack.lock を生成/更新
```
...
```bash
contextdb aggregate <root1> <root2> ...   # 複数プロジェクトの横断集計台帳（Markdown）
contextdb aggregate <root...> --out 台帳.md --type data-item
contextdb pack check <パックdir>           # パックの block 規約リリースチェック
```
...
パック自体を contextdb で正本化できる（設計メモ §3.1）。正本
（`contextdb/packs-src/<名前>/`）は doc-type / conformance-rule / style-part を
アイテムとして持ち、配布物の `documents/*.yaml` と `conformance/rules.yaml` を
そこから生成する:

```bash
contextdb pack build <正本dir> --into <配布dir>   # 正本 contextdb → 配布物 config を生成
```
...
```bash
contextdb pack migrate --root <project> --to 2.0 --dry-run   # 適用内容を確認
contextdb pack migrate --root <project> --to 2.0             # 適用（transactional）
```
...
日本 SIer 向けの実パックを `contextdb/packs/jp-sier-std/` に同梱している
...
`contextdb/packs/<名前>/` に同じ構成（pack.yaml + metamodel/ + documents/ +
...
8. 標準パック Phase 3〜4（章立て検証・移行スクリプト・横断集計ツール。
   設計: .contextdb/docs/standard-pack-design.md）
9. SQLite ビルド、検証プラグイン、docextract/fact-extractor からの

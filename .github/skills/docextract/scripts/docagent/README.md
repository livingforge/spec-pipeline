...
で取り込み、その後 **@spec-extractor** / **@corpus-qa** を使う。
...
- **横断検索（corpus-qa 用）**: **search**（本文を横断検索し出典 doc_id + location 付きで返す）
...
4. 資料を横断して調べたいときは `@corpus-qa` に質問する（出典付きで回答）。

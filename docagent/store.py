...
class Library:
    ...
    def get(self, doc_id: str) -> dict[str, Any]:
        for d in self.documents:
            if d["id"] == doc_id:
                return d
        raise DocAgentError(f"ID '{doc_id}' の文書は登録されていません")
    ...
    def add_from_result(self, result_path: str | Path, overwrite: bool = False) -> dict[str, Any]:
        ...
        if existing and not overwrite:
            raise DocAgentError(
                f"ID '{doc_id}' は既に登録済みです (上書きするには overwrite=True)"
            )
    ...
    def set_category(self, doc_id: str, category: str, force: bool = False) -> dict[str, Any]:
        ...
        if not force and category not in self.categories:
            raise DocAgentError(
                f"カテゴリ '{category}' はタクソノミーにありません。"
                f" 使用可能: {', '.join(self.categories)} (強制設定は force=True)"
            )
        doc = self.get(doc_id)
    ...
    def update(
        ...
        if category is not None:
            if not force and category not in self.categories:
                raise DocAgentError(
                    f"カテゴリ '{category}' はタクソノミーにありません: {', '.join(self.categories)}"
                )
            doc["category"] = category

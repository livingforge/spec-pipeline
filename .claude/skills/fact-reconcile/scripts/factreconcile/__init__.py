...
4. mutate plan (``plan``)        — 承認済み concept を contextdb の add-item op へ決定的に落とす

①④が決定的 (文書の提示順に依存しない)、②のみ非決定的だが必ずレビューを通る提案、
という構造で「文書を順に渡すと畳み込みが揺れる」問題を 1 箇所に封じ込める。
出力はすべて contextdb の ``status: review`` ゲートを通り、承認は人が行う。
"""

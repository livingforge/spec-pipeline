...
（`.claude/skills/contextdb/scripts/requirements.txt`）:
...
手動でコピーしたい場合は雛形 `.claude/skills/contextdb/scaffold/` を使う:

```bash
cp -r .claude/skills/contextdb/scaffold .contextdb     # サンプル入り。items/・relations/ は削除可
```
...
contextdb generate table-spec       # 指定文書だけ生成
...
名前空間）の詳細は `.claude/skills/contextdb/scaffold/README.md` と
`.claude/skills/contextdb/scaffold/metamodel.yaml` のコメントを参照。
...
同梱テストで動作確認できる: `python -m pytest .claude/skills/contextdb/scripts/tests -q`

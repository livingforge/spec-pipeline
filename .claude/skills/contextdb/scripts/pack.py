...
"""標準パックの補助操作
    ...
    python contextdb/pack.py lock                 # pack.lock を解決結果から生成/更新
    python contextdb/pack.py check <パックdir>      # パックのリリースチェック（block 規約等）
    python contextdb/pack.py build <正本dir> --into <配布dir>  # 正本 contextdb から配布物を生成
    python contextdb/pack.py migrate --to 2.0 [--dry-run]      # パック改版の移行プランを適用

pack.lock は継承チェーンの解決結果（版・内容ハッシュ）を固定する。CI は
`contextdb conform --frozen` で lock と実際の解決結果の一致を機械的に検査できる。
...
def _cmd_build(authoring: Path, into: Path) -> int:
    """パック正本 contextdb（authoring）から配布物 documents/ + conformance/ を生成し
    into（配布パックdir）へ配置する。§3.1 のパック自己正本化。"""
    import shutil
    import generate
    if not (authoring / "metamodel.yaml").is_file():
        print(f"{authoring} はパック正本 contextdb ではない。", file=sys.stderr)
        return 2
...
def main() -> int:
    ...
    if action == "build":
        if "--into" not in args:
            print("使い方: contextdb pack build <正本dir> --into <配布dir>", file=sys.stderr)
            return 2
    ...
    print("使い方: contextdb pack lock | contextdb pack check <パックdir>", file=sys.stderr)
    return 2

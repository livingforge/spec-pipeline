...
設計: .contextdb/docs/standard-pack-design.md（Phase 1 の範囲）。
"""
import os
import sys
import tempfile
...
def build(tree: dict) -> Path:
    ...
    return write_tree(Path(tempfile.mkdtemp(prefix="contextdb-std-test-")), tree)
...
def test_env_search_path(monkeypatch):
    ...
    monkeypatch.setenv("CONTEXTDB_PACK_PATH", str(base / "central"))
...
def test_lock_resolved_from_is_relative_never_absolute():
    """root 配下に無いパック（CONTEXTDB_PACK_PATH 経由）でも resolved_from は
    絶対パスにならない（ドライブ名・ユーザ名を lock に残さない）。"""
    packs_dir = build({"mini/pack.yaml": MINI_PACK,
                       "mini/metamodel/core.yaml": MINI_MM})
    root = build({"metamodel.yaml": "version: 1\nextends: mini@1.0\n"})
    old = os.environ.get("CONTEXTDB_PACK_PATH")
    os.environ["CONTEXTDB_PACK_PATH"] = str(packs_dir)
    try:
        packs, problems = resolve(root)
        ...
        if old is None:
            os.environ.pop("CONTEXTDB_PACK_PATH", None)
        else:
            os.environ["CONTEXTDB_PACK_PATH"] = old

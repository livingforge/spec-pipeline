...
データの**操作**（prep / add / set / remove）と**参照**（get / list / query /
stats / export）を提供する。
...
エージェントの基本フローは「prep → set」の 2 コマンド:

```bash
python -m docagent prep output/report_docx/result.json --json
    # 分析準備を1発で: 未登録なら登録し、解析済み判定・カテゴリ一覧・
    # 本文抜粋・次の一手をまとめた JSON を返す（登録済み ID も渡せる）
...
python -m docagent add output/report_docx/result.json     # result.json を登録のみ
...
定義は 2 段構え（コードにはハードコードしない）:

1. **既定**: パッケージ同梱の `docagent/categories.json`（8 分類: 契約・法務／
   設計・仕様／議事録／報告・レポート／見積・費用／計画・提案／マニュアル・手順／
   その他）。タクソノミーの既定を変えたいときはこのファイルを編集する。
2. **各自の上書き**: 実行ディレクトリの `store/categories.json`（git 管理外）。
   存在すればこちらが優先されるため、利用者ごとに自由にカテゴリを変更できる。
   `python -m docagent init` が既定からコピーして生成する。

`set` / `set-category` はここにある名前のみ許可（`--force` で例外設定）。編集は

...
# 並列抽出で複数スレッドが同時に初回 OCR を呼ぶと、同一言語のエンジン (数十 MB の
# モデル) を二重に構築・DL しかねない。構築を排他し、キャッシュを一度だけ埋める。
...
def _get_rapidocr_engine(lang: str):
    ...
    if engine is not None:  # 構築済みならロック無しで即返す (定常時のホットパス)
        return engine
    with _engine_lock:
        # ロック取得までに他スレッドが構築済みかを再確認する (double-checked)。

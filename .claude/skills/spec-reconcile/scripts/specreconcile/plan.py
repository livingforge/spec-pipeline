"""④ specdb mutate plan 生成 — 承認済み concept を add-item op へ決定的に落とす。

reconcile.json の concept (統合された正準概念) を、ターゲット specdb の
メタモデルに合わせた ``{"ops":[{"op":"add-item", ...}]}`` へ変換する。実適用は
行わず、既存の ``specdb mutate apply`` (トランザクション + ``status: review``
ゲート + 再検証) に委ねる。ここは決定的で、同じ reconcile.json + 同じ
メタモデルなら同じ plan を返す。

ファクトの種別 (機能要件 / データ項目 / 業務ルール …) と specdb のアイテム種別は
別語彙なので、メタモデル各種別の ``label`` と突き合わせて対応づける
(:func:`docagent.store._resolve_term` で表記揺れを吸収)。ファクトは specdb が
要求する構造化属性 (enum の type、一意な physical_name 等) を持たないため、
**埋められない必須属性が残る concept は plan に載せず skipped として報告する**
(engine error 0 を保つ)。それらは人が spec-designer で補完する。
"""
...
def load_item_types(metamodel_path: str | Path) -> dict[str, dict[str, Any]]:
    ...
    if not path.is_file():
        raise DocAgentError(
            ...
            " --metamodel でパスを指定するか --root で .specdb を指定してください"
        )
...
def map_fact_type(fact_type: str,
    ...
    """ファクト種別を specdb のアイテム種別キーへ対応づける (label 突合)。
        ...
        if missing:
            skipped.append({
                ...
                "reason": "ファクトから埋められない必須属性があります: "
                          + "、".join(missing) + " (spec-designer で補完してください)",
            })
            continue
        ops.append({
            "op": "add-item",

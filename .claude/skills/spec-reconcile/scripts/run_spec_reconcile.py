"""spec-reconcile (抽出ファクトの意味的名寄せ) のエントリポイント。

spec-reconcile スキルに同梱された specreconcile パッケージを sys.path に載せて
CLI を起動する。specreconcile は import 時に **docextract / docagent / docsummary
...
PyYAML は `plan` サブコマンドでのみ必要で、通常は specdb スキルが同じ共有 venv に
入れている (specdb の requirements)。plan を使うには specdb スキルの展開が前提。

使い方: python run_spec_reconcile.py <サブコマンド> [オプション]
"""
...
sys.path.insert(0, str(_scripts))  # specreconcile パッケージ (同梱) + _bootstrap
...
if _dx is None:
    raise SystemExit(
        "spec-reconcile: 依存する docextract / docagent が見つからない。"
        "同じプロジェクトに docextract スキルが展開されている必要がある。"
    )
if _add_sibling(_root, ("docsummary",), "docsummary") is None:
    raise SystemExit(
        "spec-reconcile: 依存する docsummary が見つからない (LLM 接続に使う)。"
...
from specreconcile.cli import main  # noqa: E402

...
5. **品質レビュー（設計書を生成する前）** — `contextdb quality` で見出し・本文の
   ...
   - **初回 authoring で `name` を作るときの規約**（`@spec-reviewer` と同一。
     守らないと `contextdb quality` の QC-NAME-CUT / QC-NAME-PREFIX / QC-NAME-SUFFIX
     で再検出される）: **体言止めの名詞句にする。目安 20 字以内。`statement` を機械的に
     切り詰めず、文の途中で切れた形（助詞止め・動詞切断・「〜の仕様」接尾）にしない。**
     `statement` の末尾節を切って `を`→`の` に変えるような組み立てをしない
     （`出力順の決定的`・`…を呼ばの仕様` のように壊れる）。名詞句として組み立て直す。

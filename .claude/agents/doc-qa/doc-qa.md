...
- `python .claude/skills/docextract/scripts/run_docagent.py {search|get|text|facts} ...`
- `Read` / `Grep`（生成物の確認のみ）
...
1. **横断検索** — 問いをキーワードに分解して検索する。**空白区切りの複数キーワードは
   ...
   ```
   python .claude/skills/docextract/scripts/run_docagent.py search "帳票 出力条件" --json
   ```
...
2. **裏取り** — 必要に応じて該当文書を深掘りする:
   ...
   python .claude/skills/docextract/scripts/run_docagent.py text <id>           # 本文（座標なし・既定 20000 字窓）
